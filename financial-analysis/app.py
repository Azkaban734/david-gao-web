...
def build_explanation(sd, r: dict, m: dict, verdict: str) -> str:
    ...
    # ── Closing (marked separately for different bubble style) ──────────
    parts.append(
    ...
    return parts  # return list so caller can render each as a bubble
...
def render_chat_bubbles(parts: list[str]) -> None:
    """Render explanation paragraphs as iMessage-style chat bubbles."""
    import re
    ...
    BUBBLE = (
        'background:#1c3a2a;border-radius:18px 18px 18px 4px;'
        'padding:13px 17px;max-width:88%;color:#dff0e8;'
    ...
    HEADER = (
        'font-size:0.78rem;color:#888;margin-bottom:6px;padding-left:2px;'
    )
    ...
    for part in parts:
        is_note = part.startswith("__NOTE__")
        text = _bold(part.replace("__NOTE__ ", ""))
        style = NOTE if is_note else BUBBLE
        rows.append(f'<div style="{style}">{text}</div>')
...
if (analyse_clicked or auto_run) and ticker_input:
    ...
    if sd.last_report_date:
        ...
        st.markdown(
            f'<div style="font-size:0.82rem;color:#999;margin:-10px 0 18px 0;">'
            f'{staleness} &nbsp;Most recent annual report: <strong>{sd.last_report_date}</strong>'
    ...
    st.caption("10-year forward projection @ 9% discount · 3% terminal growth · 30% MOS buffer")

    if m.get("Price") is None:
        st.warning(m.get("MOS Status", "No data"))
    elif m.get("OE/Share") is None:
        st.info(f"Price: **${m['Price']}** — {m['MOS Status']}")
    else:
        status = m["MOS Status"]
        if status == "IN MOS ZONE":
            st.success("✅ IN MOS ZONE — trading below intrinsic value with margin of safety")
        elif status == "APPROACHING MOS":
            st.warning("⚠️ APPROACHING MOS — near the buy zone")
        else:
            st.error("❌ ABOVE INTRINSIC VALUE")
